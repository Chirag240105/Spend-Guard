...
export async function GET(request: NextRequest) {
  try {
    ...
    const { items: policies, total } = await PolicyService.listPolicies({
      skip,
      take: limit,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!parsed.success || typeof body.naturalLanguage !== 'string')
      return NextResponse.json(
        { error: 'name, naturalLanguage and a valid compiledPolicy are required' },
        { status: 400 },
      );
    const policy = await PolicyService.createPolicy(
      typeof body.name === 'string' ? body.name : parsed.data.name,
