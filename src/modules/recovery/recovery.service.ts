...
export async function enactRecovery(
  ...
  if (action === 'HUMAN_REVIEW') {
    ...
    return prisma.humanApproval.create({ data: { agentDecisionId: decisionId } });
  }
  if (action === 'DO_NOT_RETRY') {
    await PaymentService.transition(paymentId, 'HUMAN_REVIEW');
    await PaymentService.transition(paymentId, 'REJECTED');
    return PaymentService.transition(paymentId, 'DO_NOT_RETRY');
  }
  ...
  await prisma.outboxEvent.create({
    data: {
      ...
      payload: { paymentId, delaySeconds },
    },
  });
  return { queued: true };
}
