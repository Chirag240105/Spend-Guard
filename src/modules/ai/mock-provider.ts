...
function extractPolicyName(text: string) {
  return (
    text
      .match(/^([^.!?]+)/)?.[1]
      .trim()
      .slice(0, 99) || 'Agent Spending Policy'
  );
}
...
function extractLimits(text: string): Limits {
  ...
  const patterns: Array<[keyof Limits, RegExp]> = [
    [
      'perTransaction',
      new RegExp(
        `(?:never\\s+)?spend\\s+more\\s+than\\s*${currency}${value}\\s*(?:at\\s+once|per\\s+transaction)|(?:per\\s+transaction|at\\s+once)\\s*[:\\s]*${currency}${value}`,
        'i',
      ),
    ],
    ...
    [
      'monthly',
      new RegExp(`(?:up\\s+to\\s+)?${currency}${value}\\s*(?:per\\s+month|monthly)`, 'i'),
    ],
  ];
  for (const [field, pattern] of patterns) {
    const match = pattern.exec(text);
    if (match) limits[field] = parseAmount(match[1] ?? match[2]);
  }
  return limits;
}
...
function extractCategories(text: string): Categories {
  ...
  for (const keyword of allowed)
    if (
      new RegExp(`(?:can|allow|spend|budget)\\s+(?:on\\s+)?(?:for\\s+)?${keyword}`, 'i').test(text)
    )
      categories.allowed?.push(keyword.replace('?', '').replace(/s$/, ''));
  for (const keyword of blocked)
    if (new RegExp(`(?:block|no|never|don't allow)\\s+(?:on\\s+)?${keyword}`, 'i').test(text))
...
function extractApproval(text: string) {
  const match =
    /(?:anything\s+)?above\s+[^0-9]{0,4}(\d+(?:,\d+)?)(?:\s+needs?(?:\s+my)?\s+approval|\s+approval)?|(?:approval|approve|approval required|needs? approval)\s+(?:above|for|at)\s*[^0-9]{0,4}(\d+(?:,\d+)?)/i.exec(
      text,
    );
