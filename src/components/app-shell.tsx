...
const Icons = {
  ...
  Audit: () => (
    ...
    >
      ...
      <path d="M10 2v3h3M5 7h6M5 9.5h6M5 12h4" />
    </svg>
  ),
  AI: () => (
    <svg
...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    const load = async () => {
      try {
        ...
        if (r.ok) {
          ...
          setPendingApprovals(d.approvals?.length ?? 0);
        }
      } catch {
        /* ignore */
      }
  ...
  // Don't render shell on login page or public landing
      ...
      >
        <div className="mx-auto flex h-14 max-w-[1440px] items-center justify-between px-4 sm:px-6">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Link
              href="/"
              aria-label="Go to SpendGuard landing page"
            ...
            {links.map(({ label, href, Icon }) => {
              ...
              return (
                <Link
                  ...
                  onMouseEnter={(e) => {
                    if (!active) {
                      (e.currentTarget as HTMLElement).style.background = 'var(--line-2)';
